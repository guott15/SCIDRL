...
class DANN(object):
    def __init__(self, in_dim, in_category=1, latent_dim=10, class_dim=2, lr=0.001, beta=0, gamma=0, alpha=0,
        ...
        [self.weight_pos, self.weight_neg, self.flip, self.vae, self.ae1] = self.vae_build()
    ...
    def vae_build(self):
        ...
        wc_2 = Dense(units=64, activation=None, kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                     bias_initializer='zeros', name='wc_1')(wc_1)
        wc_2 = BatchNormalization(name='wc_1_bn')(wc_2)
        wc_2 = Activation('relu')(wc_2)
        wc_3 = Dense(units=32, activation=None, kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                     bias_initializer='zeros', name='wc_2')(wc_2)
        wc_3 = BatchNormalization(name='wc_2_bn')(wc_3)
        wc_3 = Activation('relu')(wc_3)
        wc_4 = Dense(units=16, activation=None, kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                     bias_initializer='zeros', name='wc_3')(wc_3)
        wc_4 = BatchNormalization(name='wc_3_bn')(wc_4)
        wc_4 = Activation('relu')(wc_4)
        wc_5 = Dense(units=4, activation=None, kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                     bias_initializer='zeros', name='wc_4')(wc_4)
        wc_5 = BatchNormalization(name='wc_4_bn')(wc_5)
        wc_output = Dense(units=self.in_category, activation='sigmoid',
                          kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                          bias_initializer='zeros', name='weight_output')(wc_5)
        ###adversarial layer
        ...
        ad_output = Dense(units=self.in_category, activation='sigmoid',
                          kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
                          bias_initializer='zeros', kernel_regularizer=None, name='bio_output')(ad_h4)
        ###noise classifier######
        nc_output = Dense(units=self.in_category, activation='sigmoid',
                          kernel_initializer=initializers.TruncatedNormal(mean=0.0, stddev=0.1),
        ...
        #         es=0.5-K.abs(wc_output-0.5)
        es1 = 1 - wc_output
        es2 = wc_output
        vae = Model(inputs=expr_in, outputs=[dh_output, nc_output, ad_output, wc_output])
        ae1 = Model(inputs=expr_in, outputs=[z, ad_output])
        return [es1 / K.sum(es1, axis=0), es2 / K.sum(es2, axis=0), flip, vae, ae1]

    #         return [es,flip,vae,ae1]

    def weighted_ad_class(self, y_true, y_pred):
        #         adversarial_loss=K.sum(K.binary_crossentropy(y_true, y_pred)*self.weight)
        #         adversarial_loss=(K.sum(K.binary_crossentropy(y_true, y_pred)*self.weight_pos*y_true)+
        #                           K.sum(K.binary_crossentropy(y_true, y_pred)*self.weight_neg*(1-y_true)))/2
        adversarial_loss = (K.mean(K.binary_crossentropy(y_true, y_pred) * y_true) +
                            K.sum(K.binary_crossentropy(y_true, y_pred) * self.weight_neg * (1 - y_true))) / 2
        return adversarial_loss

    def compile(self, op):
        self.vae.compile(optimizer=op, loss={'ae_output': 'binary_crossentropy', 'noise_output': 'binary_crossentropy',
                                             'bio_output': self.weighted_ad_class,
    ...
    def train(self, trainX, trainY, epochs=1, batch_size=1, verbose=True, save_model=None):
        ...
        self.compile(op)
        wc_name = ['wc_3', 'wc_3_bn', 'wc_4', 'wc_4_bn', 'weight_output']
        for cnt in range(epochs):
            ...
            #             K.set_value(self.vae.layers[13].hp_lambda, K.cast_to_floatx(glr))
