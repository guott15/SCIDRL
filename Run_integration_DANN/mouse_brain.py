...
DIR="/data02/tguo/batch_effect/mouse_brain/"
...
class History(Callback):
    ...
    def on_batch_end(self, batch, logs={}):
        return
NUM=[1000,5000,10000,50000,100000,300000]
# TIME=np.zeros(len(NUM))
# k=0
for N in [300000]:
    N=np.int(N/2)
    random.shuffle(num_1)
    ...
    for v in myhistory.history.values():
        plt.plot(v)
    latent_code_mean_train=vae_.ae1.predict(expr_label)[0]
...
#     correct=vae_.ae2.predict(expr_label_1)
    ...
    k=k+1
#     np.savetxt("/data02/tguo/batch_effect/mouse_brain/data_"+str(j)+"_scidr.txt",latent_code_mean,fmt="%.5f")
#     np.savetxt("/data02/tguo/batch_effect/mouse_brain/correct_"+str(fg_lambda)+"_scidr.txt",correct,fmt="%.5f")
#     np.savetxt("/data02/tguo/batch_effect/mouse_brain/loss_"+str(fg_lambda)+".txt",v,fmt='%.6f')