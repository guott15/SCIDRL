...
types='allsame'
DIR="/data02/tguo/batch_effect/PBMC/"
data=np.loadtxt(DIR+"9tech_count_"+str(types)+".txt",dtype=np.float64)
celltype=[]
with open(DIR+"9tech_celltype_"+str(types)+".txt") as inputfile:
    for line in inputfile:
        line=line.strip("\n")
        celltype.append(line)
celltype=np.array(celltype)
batch=np.loadtxt(DIR+"9tech_batch_"+str(types)+".txt",dtype=np.int16)
L=list()
...
for j in np.arange(n):
    ...
    np.savetxt(DIR+"9tech_SCIDR_latent_"+str(types)+".txt",latent_code_mean,fmt="%.5f")
    np.savetxt(DIR+"9tech_SCIDR_correct_"+str(types)+".txt",correct,fmt="%.5f")